...
from .transformations import quaternion_from_matrix, unit_vector, euler_from_quaternion, quaternion_slerp,quaternion_matrix
...
from Utils import is_pose_matrix_close
...
def safe_zip(sequence1, sequence2): # TODO: *args
    ...
    assert len(sequence1) == len(sequence2), f'{len(sequence1)}!={len(sequence2)}'
...
def get_joint_positions(body, joints=None): # joints=None):
    if joints is None:
        n = p.getNumJoints(body)
        joints = np.arange(n)
    return np.array(list(get_joint_position(body, joint) for joint in joints))
...
def get_link_com_pose(body, link): # COM = center of mass
    if link == BASE_LINK:
...
def get_link_mesh_pose_matrix(body, link): # COM = center of mass
    trans, q_xyzw = get_link_com_pose(body,link)
    pose = quaternion_matrix(q_xyzw)
    pose[:3,3] = trans
    return pose
...
def any_link_pair_collision(body1, links1, body2, links2=None, debug=False, **kwargs):
...
def pairwise_collision(body1, body2, debug=False, **kwargs):
    if isinstance(body1, tuple) or isinstance(body2, tuple):
        body1, links1 = expand_links(body1)
        body2, links2 = expand_links(body2)
        collide = any_link_pair_collision(body1, links1, body2, links2, debug=debug, **kwargs)
        return collide

    collide = body_collision(body1, body2, **kwargs)
    return collide

def pairwise_collisions(body, obstacles, link=None, debug=False,**kwargs):
    for other in obstacles:
        if body != other:
            if pairwise_collision(body1=body, body2=other, link1=link, debug=debug, **kwargs):
                return True
    return False
...
def get_collision_fn(body, joints, obstacles, attachments, self_collisions, disabled_collisions,
                     custom_limits={},check_end_collision=True,end_conf=None, **kwargs):
    ...
    def collision_fn(q,debug=False):
        ...
        for attachment in attachments:
            attachment.assign()
        if not check_end_collision:
            if np.linalg.norm(q-end_conf)<1/180*np.pi:
                return False

        for link1, link2 in check_link_pairs:
            # Self-collisions should not have the max_distance parameter
            if pairwise_link_collision(body, link1, body, link2): #, **kwargs):
                print(f'Self Collision: body{body}, link{link1} and link{link2}')
                return True
        for body1, body2 in check_body_pairs:
            if pairwise_collision(body1, body2, debug=debug, **kwargs):
                return True
        return False
    return collision_fn
...
def plan_joint_motion(body, joints, end_conf, obstacles=[], attachments=[],
                      self_collisions=True, disabled_collisions=set(),
                      weights=None, resolutions=None, max_distance=MAX_DISTANCE, custom_limits={}, ignore_all_collsion=False, collision_fn=None,check_end_collision=True, **kwargs):
    ...
    if collision_fn is None:
        if ignore_all_collsion:
            def ignore_collision(q):
                return False
            collision_fn = ignore_collision
        else:
            collision_fn = get_collision_fn(body, joints, obstacles, attachments, self_collisions, disabled_collisions,
                                        custom_limits=custom_limits, max_distance=max_distance,check_end_collision=check_end_collision,end_conf=end_conf)
    ...
    #     return None
    print('Start birrt')
    return birrt(start_conf, end_conf, distance_fn, sample_fn, extend_fn, collision_fn,check_end_collision=check_end_collision, **kwargs)
...
def add_fixed_constraint(body, robot, robot_link=BASE_LINK, max_force=None):
    ...
    #body_pose = get_link_com_pose(body, link=body_link)
    #end_effector_pose = get_link_pose(robot, robot_link)
    end_effector_pose = get_link_com_pose(robot, robot_link)
...
def joint_controller(body, joints, target, tolerance=1e-3, timeout=INF, dt=0,  **kwargs):
    ...
    while 1:
        ...
        if time_elapsed>=timeout:
            break
        if (np.abs(np.array(positions)-np.array(target))<tolerance).all():
            break
...
def inverse_kinematics_helper(robot, link, target_pose, null_space=None):
    ...
    if null_space is not None:
        ...
        kinematic_conf = p.calculateInverseKinematics(robot, link, targetPosition=target_point, targetOrientation=target_quat, physicsClientId=CLIENT)
...
def is_pose_close(pose, target_pose, pos_tolerance=1e-3, ori_tolerance=1e-3*np.pi):
    ...
    if quat is None:
        return False
    axis = np.array([0,0,1]).reshape(3,1).astype(float)
    R = quaternion_matrix(quat)[:3,:3]
    R_target = quaternion_matrix(target_quat)[:3,:3]
    dot = np.dot((R@axis).reshape(-1), (R_target@axis).reshape(-1))
    if dot<1-np.cos(1/180.0*np.pi):
        return False
    return True

def inverse_kinematics(robot, link, target_pose, max_iterations=200, custom_limits={}, ignore_joint_limits=False, **kwargs):
    '''
    @target_pose: trans, q_xyzw
    -----------------------
    @kinematic_conf: returns a list of joint positions for each degree of freedom, see p.calculateInverseKinematics
    '''
    state_id = p.saveState()
    trans = target_pose[:3,3]
    q_xyzw = quaternion_from_matrix(target_pose)
    ...
    for iterations in range(max_iterations):
        ...
        kinematic_conf = inverse_kinematics_helper(robot, link, (trans,q_xyzw))
        if kinematic_conf is None:
            p.restoreState(stateId=state_id)
            p.removeState(state_id)
            return None
        set_joint_positions(robot, movable_joints, kinematic_conf)
        if is_pose_close(get_link_pose(robot, link), (trans,q_xyzw), **kwargs):
            if not ignore_joint_limits:
                if all_between(lower_limits, kinematic_conf, upper_limits):
                    p.restoreState(stateId=state_id)
                    p.removeState(state_id)
                    return kinematic_conf
...
def plan_cartesian_motion(robot, first_joint, target_link, waypoint_poses,
    ...
    '''
    @waypoint_poses: list of (trans,q_xyzw)
    '''
...
def plan_cartesian_motion_ikfast(robot, arm_joints, ee_link, waypoint_poses, get_ik_func, custom_limits={}, obstacles=[], attachments=[],collision_fn=None,check_end_collision=True,**kwargs):
    '''Modiefied from `plan_cartesian_motion`
    @waypoint_poses: list of (trans,q_xyzw) of ee link in world
    '''
    state_id = p.saveState()
    trans,q_xyzw = get_pose(robot)
    base_in_world = quaternion_matrix(q_xyzw)
    base_in_world[:3,3] = trans
    init_joint_positions = get_joint_positions(robot,arm_joints)
    lower_limits, upper_limits = get_custom_limits(robot, arm_joints, custom_limits)
    solutions = []
    for (trans,q_xyzw) in waypoint_poses:
        ee_in_world = quaternion_matrix(q_xyzw)
        ee_in_world[:3,3] = trans
        ee_in_base = np.linalg.inv(base_in_world)@ee_in_world
        sols = get_ik_func(trans_list=ee_in_base[:3,3], rot_list=ee_in_base[:3,:3])
        if len(sols)==0:
            p.restoreState(stateId=state_id)
            p.removeState(state_id)
            print('plan_cartesian_motion_ikfast::Failed to find sols in middle')
            return None

        feasible_sols = []
        for i_sol,sol in enumerate(sols):
            set_joint_positions(robot, arm_joints, sol)
            trans,q_xyzw = get_link_pose(robot, ee_link)
            cur_ee_in_world = quaternion_matrix(q_xyzw)
            cur_ee_in_world[:3,3] = trans
            if is_pose_matrix_close(cur_ee_in_world,ee_in_world,trans_tol=0.001,rot_tol=2):
                kinematic_conf = get_joint_positions(robot,arm_joints)
                if all_between(lower_limits, kinematic_conf, upper_limits):
                    collision = False
                    for obs in obstacles:
                        if pairwise_collision(robot,obs):
                            collision = True
                            break
                        for att in attachments:
                            if pairwise_collision(att.child,obs):
                                collision = True
                                break
                    if i_sol==len(sols)-1:
                        if not check_end_collision:
                            collision = False
                    if collision:
                        print("plan_cartesian_motion_ikfast::collision")
                        return None
                    feasible_sols.append(kinematic_conf)
                # else:
                    # print(f"joints {kinematic_conf} out of limit, lower {lower_limits}, higher {upper_limits}")
        if len(feasible_sols)==0:
            p.restoreState(stateId=state_id)
            p.removeState(state_id)
            print('plan_cartesian_motion_ikfast::feasible_sols is empty')
            return None

        feasible_sols = np.array(feasible_sols)
        if len(solutions)==0:
            last_conf = init_joint_positions
        else:
            last_conf = solutions[-1]
        best_id = np.linalg.norm(feasible_sols-np.array(last_conf).reshape(1,feasible_sols.shape[1]), axis=-1).reshape(-1).argmin()
        best_sol = feasible_sols[best_id]
        if np.abs(best_sol-last_conf).max()>30/180*np.pi:
            print('plan_cartesian_motion_ikfast sol too much different from last conf')
            return None

...
from .utils import get_bodies,get_pose, set_pose, get_movable_joints, \
    ...
    step_simulation, refine_path, plan_direct_joint_motion, get_joint_positions, dump_world, wait_if_gui,body_collision
...
class BodyPath(object):
    ...
    def control(self, real_time=False, dt=0, joints=None, timeout=9999999):
        ...
        if real_time:
            ...
            disable_real_time()
        print("BodyPath self.path#={}".format(len(self.path)))
        for ipath,values in enumerate(self.path):
            if ipath%max(1,(len(self.path)//5))==0:
                print(f'BodyPath.control {ipath}/{len(self.path)}')
...
class Command(object):
    ...
    def execute(self, time_step=0.05, obstacles=[]):
        '''Set link positions without simulation
        '''
        for i, body_path in enumerate(self.body_paths):
            for j in body_path.iterator():
                #time.sleep(time_step)
                for obs in obstacles:
                    if pairwise_collision(body_path.body,obs):
                        return False
                    for att in body_path.attachments:
    ...
    def control(self, real_time=False, dt=0, timeout=99999): # TODO: real_time
        for i_path,body_path in enumerate(self.body_paths):
            print('control paths {}/{}'.format(i_path,len(self.body_paths)))
            body_path.control(real_time=real_time, dt=dt, timeout=timeout)
    ...
    def fn(conf1, conf2, fluents=[]):
        ...
        if teleport:
            ...
            if path is None:
                ...
                return None
        command = Command([BodyPath(robot, path, joints=conf2.joints, attachments=attachments)])
        p.restoreState(stateId=state_id)
        p.removeState(state_id)
        return (command,)
    return fn
